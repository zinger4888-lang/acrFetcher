...
BUNDLED_ART_PATH = Path(__file__).with_name("menu_art.txt")
    ...
    async def run_once(headless: bool) -> tuple[str, str]:
        ...
        async with async_playwright() as p:
            ...
            def match_missed(tnorm: str, lines: list) -> tuple[bool, str]:
                # hardcoded "missed" phrases: page loaded but already claimed.
                hard_succ = [
                ...
                for hs in hard_succ:
                    if hs in tnorm:
                        detail = next((l for l in lines if hs in l.lower()), hs)
                        return True, detail
...
async def watch_multi(cfg: dict, *, resume: bool = False) -> None:
    ...
    csv_path = Path(csv_raw).expanduser() if csv_raw else Path(__file__).with_name("accounts.csv")
